...
//        var_dump($categoryArray);exit;
    ...
    public function headerAction($previewAs, $classroomId )
    {
        ...
        $currentTime = time();
        $createdTime = $classroom['createdTime'];
        $finishedTimeStart = strtotime(date("Y-m-d", $createdTime));
        $finishedTimeEnd = strtotime(date("Y-m-d", $currentTime));
        $finishedCourseNum = $this->getClassroomService()->searchCourseCount(array("targetType" => "classroom", "courseIds" => $courseIds, "startTime" => $finishedTimeStart, "endTime" => $finishedTimeEnd, "status" => "finished"));
        $finishedStatus = $this->calculateUserLearnProgress($classroom, $user->id);
        $firstUserShouldLearnCourse = $this->firstUserShouldLearnCourse($classroom, $user->id);
        if ($member && !$member["locked"]) {
            return $this->render("ClassroomBundle:Classroom:classroom-join-header.html.twig", array(
                ...
                'firstUserShouldLearnCourse' => $firstUserShouldLearnCourse
            ));
        }
    ...
    public function showAction(Request $request, $id)
    {
        ...
        $this->getClassroomService()->hitClassroom($id);
    ...
    private function firstUserShouldLearnCourse($classroom, $userId)
    {
        $courses            = $this->getClassroomService()->findActiveCoursesByClassroomId($classroom['id']);
        $courseIds          = ArrayToolkit::column($courses, 'id');
        $findUnlearnCourses = array();

        foreach ($courseIds as $key => $value) {
            $learnedCourses = $this->getCourseService()->findLearnedCoursesByCourseIdAndUserId($value, $userId);
            if (empty($learnedCourses)) {
                $findUnlearnCourses[] = $value;
            }
        }
        $courses =array();
        if(!empty($findUnlearnCourses)){
            $courses = $this->getCourseService()->getCourse($findUnlearnCourses[0]);
