...
class ClassroomServiceImpl extends BaseService implements ClassroomService
{
    ...
    public function addCoursesToTravel($travelId, $courseIds)
    {
//        $this->tryManageClassroom($classroomId);
//        $this->getClassroomDao()->getConnection()->beginTransaction();
...
//        var_dump($courseIds);exit;
        ...
        try {
            //find Existing Courses, open it and active it
            $allExistingCourses   = $this->findCoursesByTravelId($travelId);


//            var_dump($allExistingCourses);exit;
            $existCourseIds       = array();
            $existCourseParentIds = array();
            ...
            foreach ($courseIds as $val){
                ...
                $this->getClassroomCourseDao()->addCourse($travelCourse,true);
...
//            foreach ($sameCourseIds as $key => $courseId) {
//                $courseId = $existCourseIds[$courseId];
//                $this->getClassroomCourseDao()->updateTravelByParam(array('classroomId' => $travelId, 'courseId' => $courseId), array('disabled' => 0));
...
//            $this->getClassroomDao()->getConnection()->rollback();
    ...
    public function hitClassroom($id)
    {
        ...
        $this->getClassroomDao()->waveClassroom($id, 'hitNum', +1);
    }
    ...
    public function setClassroomCostPrice($classroomId, array $courseIds, $costPrice)
    {
        $courses        = $this->findCoursesByClassroomId($classroomId);
        $existCourseIds = ArrayToolkit::column($courses, 'id');
    ...
    public function updateTravelCourses($travelId, $activeCourseIds)
    {
        try {
            $courses        = $this->findActiveCoursesByTravelId($travelId);
            $courses        = ArrayToolkit::index($courses, 'id');
            $existCourseIds = ArrayToolkit::column($courses, 'id');
            $diff = array_diff($existCourseIds, $activeCourseIds);
            //删除课程
            if (!empty($diff)) {
                foreach ($diff as $courseId) {
                    $this->getClassroomCourseDao()->deleteCourseByTravelIdAndCourseId($travelId, $courseId);
                }
            }
            $this->refreshTravelCoursesSeq($travelId, $activeCourseIds);
    ...
    private function refreshTravelCoursesSeq($classroomId, $courseIds)
    {
        $seq = 1;

        foreach ($courseIds as $key => $courseId) {
            $travelCourse = $this->findTravelCourse($classroomId, $courseId);
            $this->getClassroomCourseDao()->updateTravel($travelCourse['id'], array('seq' => $seq));
            $seq++;
        }
    }
    ...
    {
        return $this->getClassroomCourseDao()->findTravelCourse($travelId, $courseId);
    }
    ...
    {
        $travelCourses = $this->getClassroomCourseDao()->findCoursesByTravelId($travelId);
        $courseIds        = ArrayToolkit::column($travelCourses, "courseId");
        $courses          = $this->getCourseService()->findCoursesByIds($courseIds);
        $courses          = ArrayToolkit::index($courses, "id");
        $sordtedCourses   = array();

        foreach ($travelCourses as $key => $classroomCourse) {
            $sordtedCourses[$key] = $courses[$classroomCourse["courseId"]];
        }
    ...
    {
        $travelCourses = $this->getClassroomCourseDao()->findActiveCoursesByTravelId($travelId);
        $courseIds        = ArrayToolkit::column($travelCourses, 'courseId');
