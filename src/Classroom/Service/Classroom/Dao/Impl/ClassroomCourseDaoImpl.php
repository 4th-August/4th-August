...
class ClassroomCourseDaoImpl extends BaseDao implements ClassroomCourseDao
{
    ...
    public function addCourse($course,$isTravel=false)
    {
        ...
        if($isTravel){
            $affected = $this->getConnection()->insert('travel_courses', $course);
        }else{
    ...
    {
        $this->getConnection()->update('travel_courses', $fields, array('id' => $id));
    ...
    {
        $this->getConnection()->update('travel_courses', $fields, $params);
    }
    ...
    {
        return $this->getConnection()->delete('travel_courses', array('travelId' => $travelId, 'courseId' => $courseId));
    }
    ...
    {
        $sql = "SELECT * FROM travel_courses where travelId = ? AND courseId = ? LIMIT 1";

        return $this->getConnection()->fetchAssoc($sql, array($travelId, $courseId)) ?: null;
    }
    ...
    {
        $sql = "SELECT * FROM travel_courses WHERE travelId = ? ORDER BY seq ASC;";

        return $this->getConnection()->fetchAll($sql, array($travelId)) ?: array();
    }
    ...
    {
        $sql = "SELECT * FROM travel_courses WHERE travelId = ? AND disabled = 0 ORDER BY seq ASC;";
