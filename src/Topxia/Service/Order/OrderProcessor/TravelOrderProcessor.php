...
class TravelOrderProcessor extends BaseProcessor implements OrderProcessor
{
    ...
    public function doPaySuccess($success, $order)
    {
        ...
        //加入赠送课程
        $courses = $this->getClassroomService()->findActiveCoursesByTravelId($order['targetId']);
        $info = array(
            'orderId' => $order['id'],
            'remark'  => empty($order['data']['note']) ? '' : $order['data']['note'],
        );
        foreach ($courses as $course){
            if (!$this->getCourseService()->isCourseStudent($course['id'], $order['userId'])) {
                $this->getCourseService()->becomeStudent($course['id'], $order['userId'], $info);
            }
        }
