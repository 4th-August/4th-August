...
class ThreadServiceImpl extends BaseService implements ThreadService
{
    ...
    public function isliked($userId, $threadId)
    {
        $thread = $this->getThreadLikeDao()->getThreadByUserIdAndThreadId($userId, $threadId);
        if (empty($thread)) {
            return false;
        } else {
            return true;
    ...
    public function threadLike($userId, $threadId)
    {

        $thread = $this->getThread($threadId);
        if (empty($thread)) {
            throw $this->createServiceException('话题不存在，点赞失败！');
        }

        $collectThread = $this->getThreadLikeDao()->getThreadByUserIdAndThreadId($userId, $threadId);
        if (!empty($collectThread)) {
            throw $this->createServiceException('不允许重复点赞!');
        }

        return $this->getThreadLikeDao()->addThreadLike(array(
            "userId" => $userId,
            "threadId" => $threadId,
            "createdTime" => time()));
    }
    ...
    public function unThreadLike($userId, $threadId)
    {
        $thread = $this->getThread($threadId);
        if (empty($thread)) {
            throw $this->createServiceException('话题不存在，取消点赞失败！');
        }
        $collectThread = $this->getThreadLikeDao()->getThreadByUserIdAndThreadId($userId, $threadId);
        if (empty($collectThread)) {
            throw $this->createServiceException('不存在此收藏关系，取消收藏失败！');
        }
        return $this->getThreadLikeDao()->deleteThreadLikeByUserIdAndThreadId($userId, $threadId);
    }
    ...
    {
        return $this->getThreadLikeDao()->searchThreadLikesCount($conditions);
    }
    ...
    protected function getThreadLikeDao()
    {
        return $this->createDao('Group.ThreadLikeDao');
    }
}
