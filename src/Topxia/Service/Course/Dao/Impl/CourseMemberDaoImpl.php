...
class CourseMemberDaoImpl extends BaseDao implements CourseMemberDao
{
    ...
    public function findMembersNotInClassroomByUserIdAndRole($userId, $role, $start, $limit, $onlyPublished = true,$type='normal')
    {
        $this->filterStartLimit($start, $limit);
        ...
        if($onlyPublished){
            $sql .= " AND c.status = 'published' AND c.type = ? ";
        }
    ...
    public function findAllMemberByUserIdAndRole($userId, $role, $onlyPublished = true, $courseType = 'normal')
    {
        $this->filterStartLimit($start, $limit);

        $sql  = "SELECT m.* FROM {$this->table} m ";
        $sql.= ' JOIN  '. CourseDao::TABLENAME . ' AS c ON m.userId = ? ';
        $sql .= " AND m.role =  ? AND m.courseId = c.id AND c.type = ? ";
        if($onlyPublished){
            $sql .= " AND c.status = 'published' ";
        }
        // $sql .= " ORDER BY createdTime DESC LIMIT {$start}, {$limit}";

        return $this->getConnection()->fetchAll($sql, array($userId, $role,$courseType));
    }
