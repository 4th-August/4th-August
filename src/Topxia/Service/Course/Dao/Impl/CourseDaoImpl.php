...
class CourseDaoImpl extends BaseDao implements CourseDao
{
    ...
    {
        $affected = $this->getConnection()->insert('dz_data', $conditions);
    ...
    {
        $sql = "SELECT * FROM dz_data WHERE TYPE = 1 LIMIT 3";

        $data=$this->getConnection()->fetchAll($sql);

        $sql = "SELECT * FROM dz_data WHERE TYPE = 2 LIMIT 12";

        $data2=$this->getConnection()->fetchAll($sql);
    ...
    public function searchRandomCourses($type,$limit)
    {
        ...
        $sql = "SELECT * FROM {$this->getTablename()} WHERE parentId = 0 AND status = 'published' AND  type = ? order by rand() limit {$limit}";
    ...
    public function searchTravelRandomCourses($categoryIds,$limit)
    {
        ...
        $sql = "SELECT * FROM {$this->getTablename()} WHERE parentId = 0 AND status = 'published' AND  type = 'travel' AND categoryId IN ( $categoryIds ) order by rand() limit {$limit}";
//var_dump($sql);exit;
//        $sql = "SELECT * FROM {$this->getTablename()} WHERE parentId = 0 AND status = 'published' AND  type = 'travel' order by rand() limit {$limit}";
    ...
    public function searchNormalRandomCourses($categoryIds,$limit)
    {
        ...
        $sql = "SELECT * FROM {$this->getTablename()} WHERE parentId = 0 AND status = 'published' AND  type = 'normal' AND categoryId IN ( $categoryIds ) order by rand() limit {$limit}";
    ...
    protected function _createSearchQueryBuilder($conditions)
    {
        ...
        $builder = $this->createDynamicQueryBuilder($conditions)
                        ...
                        ->andWhere('travelRange = :range')
                        ->andWhere('status = :status')
                        ->andWhere('type = :type')
                        ->andWhere('price = :price')
                        ->andWhere('price > :price_GT')
                        ->andWhere('price >= :tpriceGreater')
                        ->andWhere('price <= :tpriceLess')
                        ...
                        ->andWhere('travelStartTime >= :travelStartTime')
                        ->andWhere('travelStartTime <= :travelEndTime')
                        ...
                        ->andWhere('categoryId != :excludeCategoryId')
                        ->andWhere('categoryId NOT IN (:excludeCategoryIds)')
                        ->andWhere('type NOT IN (:excludeTypes)');
