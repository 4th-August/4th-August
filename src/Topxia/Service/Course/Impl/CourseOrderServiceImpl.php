...
class CourseOrderServiceImpl extends BaseService implements CourseOrderService
{
    ...
    public function createOrder($info)
    {
        ...
        try {
            ...
            if (empty($course)) {
                if($course['type'] == 'product'){
                    throw $this->createServiceException('商品不存在，操作失败。');
                }
            ...
            if($course['type'] == 'product'){
                $order['title'] = "购买商品《{$course['title']}》";
                $order['targetType'] = 'product';
            }else if($course['type'] == 'travel'){
                $order['title'] = "购买游学《{$course['title']}》";
                $order['targetType'] = 'travel';
            }else{
            ...
            if($course['type'] == 'product'){
                $order['productType'] = $info["productType"];
                $order['productNum'] = $info['productNum'];
                $order['productAddress'] = $info['productAddress'];
            ...
            if (empty($order)) {
                if($course['type'] == 'product'){
                    throw $this->createServiceException('创建商品订单失败！');
                }
            ...
            if($course['type'] == 'product') {
                ...
                foreach ($course['travelDetail']['carousel'] as $kk => $vv) {
                    if ($vv['title'] == $order['productType']) {
                        $num=$vv['number']-$order['productNum'];
                        if($num>0){
                            $course['travelDetail']['carousel'][$kk]['number']=$num;
                            $data['travelDetail'] = json_encode( $course['travelDetail']);
    ...
    public function doSuccessPayOrder($id)
    {
        ...
        if (empty($order) || ($order['targetType'] != 'course' && $order['targetType'] != 'travel')) {
