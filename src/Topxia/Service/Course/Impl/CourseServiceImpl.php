...
class CourseServiceImpl extends BaseService implements CourseService
{
    ...
    public function searchRandomCourses($type,$limit)
    {
        return CourseSerialize::unserializes($this->getCourseDao()->searchRandomCourses($type,$limit));
    }

    public function searchTravelRandomCourses($categoryIds,$limit)
    {
        return CourseSerialize::unserializes($this->getCourseDao()->searchTravelRandomCourses($categoryIds,$limit));
    }

    public function searchNormalRandomCourses($categoryIds,$limit)
    {
        return CourseSerialize::unserializes($this->getCourseDao()->searchNormalRandomCourses($categoryIds,$limit));
    }


    public function addDz($conditions){

        return $this->getCourseDao()->addDz($conditions);

    }

    public function rmDz(){
        return $this->getCourseDao()->rmDz();
    }

    public function getDz(){
        return $this->getCourseDao()->getDz();
    }
    ...
    protected function _prepareCourseConditions($conditions)
    {
        ...
        if (isset($conditions['month']) && $conditions['month']!='all') {
            $conditions['travelStartTime'] = strtotime($conditions['month'].'-1');
            $firstDay=$conditions['month'].'-1';
            $conditions['travelEndTime']    =strtotime("$firstDay +1 month -1 seconds");
        }

        if(isset($conditions['tprice']) && $conditions['tprice']!='all'){
            if($conditions['tprice']==1){
                $conditions['tpriceGreater'] =0;
                $conditions['tpriceLess']    =10000;
            }
            if($conditions['tprice']==2){
                $conditions['tpriceGreater'] =10000;
                $conditions['tpriceLess']    =19999;
            }
            if($conditions['tprice']==3){
                $conditions['tpriceGreater'] =20000;
                $conditions['tpriceLess']    =29999;
            }
            if($conditions['tprice']==4){
                $conditions['tpriceGreater'] =30000;
                $conditions['tpriceLess']    =150000;
            }
            unset($conditions['tprice']);
        }
    ...
    // TODO searchCourse
    public function findUserLearnCoursesNotInClassroom($userId, $start, $limit, $onlyPublished = true,$type='normal')
    {
        $members = $this->getMemberDao()->findMembersNotInClassroomByUserIdAndRole($userId, 'student', $start, $limit, $onlyPublished,$type);
    ...
    public function findUserTeachCourseCount($conditions, $onlyPublished = true , $courseType = 'normal')
    {
        $members = $this->getMemberDao()->findAllMemberByUserIdAndRole($conditions['userId'], 'teacher', $onlyPublished,$courseType);
    ...
    public function findUserTeachCourses($conditions, $start, $limit, $onlyPublished = true, $courseType = 'normal')
    {
        $members = $this->getMemberDao()->findAllMemberByUserIdAndRole($conditions['userId'], 'teacher', $onlyPublished,$courseType);
        unset($conditions['userId']);
    ...
    public function createCourse($course)
    {
        ...
        if($course['type']=='product'){
            $this->getLogService()->info('product', 'create', "创建商品《{$course['title']}》(#{$course['id']})");
        }else{
    ...
    public function updateCourse($id, $fields)
    {
        ...
        if($course['type']=='product'){
            if( isset($fields['income']) && isset($fields['studentNum'])){
                $this->getLogService()->info('product', 'update', "更新商品《{$course['title']}》(#{$course['id']})的销量", $fields);
            }else{
                $this->getLogService()->info('product', 'update', "更新商品《{$course['title']}》(#{$course['id']})的信息", $fields);
            }
        }else{
    ...
    protected function _filterCourseFields($fields)
    {
        ...
        if (!empty($fields['travelStartTime'])) {
            $diff=date_diff(date_create($fields['travelStartTime']),date_create($fields['travelEndTime']));
            $fields['travelDays']=$diff->days;
            $fields['travelStartTime'] = strtotime($fields['travelStartTime']);
        }

        if (!empty($fields['travelEndTime'])) {
            $fields['travelEndTime'] = strtotime($fields['travelEndTime']);
    ...
    public function publishCourse($id, $source = 'course')
    {
        ...
        if($course['type']=='product'){
            $this->getLogService()->info('product', 'publish', "发布商品《{$course['title']}》(#{$course['id']})");
        }else{
    ...
    public function setCourseCostPrice($courseId, $currency, $costPrice)
    {
        if (!in_array($currency, array('coin', 'default'))) {
            throw $this->createServiceException("货币类型不正确");
        }
