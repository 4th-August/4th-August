...
class UserServiceImpl extends BaseService implements UserService
{
    ...
    public function updateAddress($id,$conditions)
    {
        return $this->getAddressDao()->updateAddress($id,$conditions);
    }

    public function deleteAddress($id){
        return $this->getAddressDao()->deleteAddress($id);
    }

    public function addAddress($conditions)
    {
        return $this->getAddressDao()->addAddress($conditions);
    }

    public function getAddressByUserId($id)
    {
        return $this->getAddressDao()->getAddressByUserId($id);
    }
    ...
    public function changePicture($userId, $data)
    {
        $user = $this->getUser($userId);

        if (empty($user)) {
            throw $this->createServiceException('用户不存在，背景更新失败！');
        }
        ...
        $fields = array(
            'picture'  => $files[$fileIds["large"]["id"]]["uri"]
        );
        ...
        $fileService = $this->getFileService();
        array_map(function ($old) use ($fileService) {
            if (!empty($old)) {
                $fileService->deleteFileByUri($old);
            }
        }, $old);
    ...
    {
        return $this->createDao('User.UserAddressDao');
    }
