            ...
            $.get(this.get('courseUri') + '/lesson/' + id, function(lesson) {
                ...
                if (lesson.mediaSource == 'iframe') {
                    ...
                    function generateHtml() {
                        ...
                        $replayGuid += "&nbsp;&nbsp;&nbsp;&nbsp;"+'开始直播需在客户端软件中点击';
                        $replayGuid += "<span style='color:red'>"+'开始串流'+"</span>";
                        $replayGuid += '，一旦开始推流视频会';
                        $replayGuid += "<span style='color:red'>自动录制</span>";
                        $replayGuid += '，点击';
                        $replayGuid += "<span style='color:red'>停止串流</span>";
                        $replayGuid += '即结束本次课程，自动生成课程回放。';
                        $replayGuid += "<br>";
                        ...
                        if (0 < startLeftSeconds && startLeftSeconds < 7200) {
                            $liveNotice = "<p>直播将于 <strong>" + liveStartTimeFormat + "</strong> 开始，于 <strong>" + liveEndTimeFormat + "</strong> 结束，请在开始时刷新该页面。</p>";
                            var url = self.get('courseUri') + '/lesson/' + id + '/live_entry';
                            if (lesson.isTeacher) {
                                $countDown = $replayGuid;
                                $countDown += "<p>还剩" + hours + "小时" + minutes + "分钟" + seconds + "秒&nbsp;";
                            } else {
                                $countDown = "<p>还剩" + hours + "小时" + minutes + "分钟" + seconds + "秒&nbsp;";
                            }
                        };
