define(function(require, exports, module) {
	...
	exports.run = function() {
        ...
        $(".address-finish .addr:not(.add-addr)").hover(function () {
        ...
        $(".address-finish .addr:not(.add-addr)").click(function () {
        	$(".addr-selected").removeClass("addr-selected")
            $(this).addClass("addr-selected");
        	$("[name=address]").val($(this).data('id'));
        });

        $("#show_all_address").click(function () {
        	var index=$("#show_all_address").data("num");
        	if(index>=3 && index<6){
        		$(".addr-list").css('height',"240px");
			}
            if(index>=6 && index<9){
                $(".addr-list").css('height',"360px");
            }
            if(index>=9 && index<12){
                $(".addr-list").css('height',"480px");
            }
            $("#show_all_address").hide();
        });
