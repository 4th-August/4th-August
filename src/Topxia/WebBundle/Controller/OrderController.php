...
class OrderController extends BaseController
{
    public function showAction(Request $request)
    {
        ...
        $num   =  intval($request->query->get('num'));

        if (empty($targetType) || empty($targetId) || !in_array($targetType, array("course", "vip", "classroom","product","travel"))) {
        ...
        if(isset($orderInfo['course'])){
            if($orderInfo['course']['type']=='product'){
                $orderInfo['NoAddress']="0";
                $orderInfo['address']=$this->getUserService()->getAddressByUserId($currentUser['id']);
                if(count($orderInfo['address']) ==0){
                    $orderInfo['NoAddress']="1";
                }
            }
        }
...
//        if($orderInfo[])
    ...
    public function detailAction(Request $request, $id)
    {
        ...
        if($order['targetType'] == 'product'){
            $order['productAddress']=json_decode($order['productAddress'],true);
        }
