...
class GroupThreadController extends BaseController
{
    ...
    public function likeAction(Request $request, $threadId)
    {
        ...
        $isSetThread=$this->getThreadService()->threadLike($user['id'],$threadId);


        return $this->createJsonResponse(true);
    }

    public function unlikeAction(Request $request, $threadId)
    {
        $user = $this->getCurrentUser();
        if (!$user->isLogin()) {
            return $this->createErrorResponse($request, 'not_login', "您尚未登录，不能收藏话题！");
        ...
        $isSetThread=$this->getThreadService()->unThreadLike($user['id'], $threadId);
    ...
    public function groupThreadIndexAction(Request $request, $id, $threadId)
    {
        ...
        $liked = $this->getThreadService()->isliked($user['id'],$threadMain['id']);
