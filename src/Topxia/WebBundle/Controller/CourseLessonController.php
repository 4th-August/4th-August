    ...
    public function showAction(Request $request, $courseId, $lessonId)
    {
        ...
        if ($lesson['type'] == 'live') {
            ...
            if($lesson['endTime']<time()){
                ...
                $json['ReplayUrl']=$Url='http://record.yxwps.com/record/yxwps/'.$lesson['liveProvider'].'.m3u8';

            }else{

                $key='/yxwps/'.$lesson['liveProvider'].'.m3u8-'.$lesson['endTime'].'-0-0-6JD0pQydYT3BzBcmzQk2oaciAVhiIe3M';

                $Url='http://live.yxwps.com/yxwps/'.$lesson['liveProvider'].'.m3u8?auth_key='.$lesson['endTime'].'-0-0-'.md5($key);
