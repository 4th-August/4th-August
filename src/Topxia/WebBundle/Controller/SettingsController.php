...
class SettingsController extends BaseController
{
    ...
    {
        $user = $this->getCurrentUser();
        return $this->render('TopxiaWebBundle:Settings:picture.html.twig', array(
            'user'   => $this->getUserService()->getUser($user['id']),
        ));
    }
    ...
    {
        $user = $this->getCurrentUser();
        $address=$this->getUserService()->getAddressByUserId($user['id']);
        return $this->render('TopxiaWebBundle:Settings:address.html.twig', array(
            'user'   => $this->getUserService()->getUser($user['id']),
            'address' => $address
        ));
    }
    ...
    public function editAddressAction(Request $request,$id)
    {
        ...
        $user                 = $this->getCurrentUser();
        $address= $this->getUserService()->getUserAddress($id);
        if($address['userId'] !== $user['id']){
            throw $this->createAccessDeniedException('权限错误！');
        }
        ...
        if ($request->getMethod() == 'POST') {
            ...
            if( !is_null($data['name']) && !is_null($data['city']) && !is_null($data['tel']) && !is_null($data['address']) && !is_null($data['isD'])){
                $this->getUserService()->updateAddress($id, $data);
            }else{
                throw $this->createAccessDeniedException('参数错误！');
            }
            return $this->redirect($this->generateUrl('settings_address'));

        }

        return $this->render('TopxiaWebBundle:Settings:edit-address.html.twig', array(
            'address' => $address,
        ));
    }
    ...
    public function addAddressAction(Request $request)
    {
        ...
        if ($request->getMethod() == 'POST') {
            ...
            if( !is_null($data['name']) && !is_null($data['city']) && !is_null($data['tel']) && !is_null($data['address']) && !is_null($data['isD'])){
                $this->getUserService()->addAddress($data);
            }else{
                throw $this->createAccessDeniedException('参数错误！');
            }
            return $this->redirect($this->generateUrl('settings_address'));
        ...
        return $this->render('TopxiaWebBundle:Settings:add-address.html.twig');
    }
    ...
    public function deleteAddressAction(Request $request,$id)
    {
        ...
        $result=$this->getUserService()->deleteAddress($id);
        if($result){
    ...
    public function pictureCropAction(Request $request)
    {
        ...
        if ($request->getMethod() == 'POST') {
            ...
            $this->getUserService()->changePicture($currentUser['id'], $data["images"]);
            return $this->redirect($this->generateUrl('settings_picture'));
        }

        $fileId                                      = $request->getSession()->get("fileId");
        list($pictureUrl, $naturalSize, $scaledSize) = $this->getFileService()->getImgFileMetaInfo($fileId, 1400, 340);

        return $this->render('TopxiaWebBundle:Settings:picture-crop.html.twig', array(
            'pictureUrl'  => $pictureUrl,
            'naturalSize' => $naturalSize,
            'scaledSize'  => $scaledSize
        ));
