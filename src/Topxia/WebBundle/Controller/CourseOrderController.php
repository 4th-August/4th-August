...
class CourseOrderController extends OrderController
{
    ...
    public function buyAction(Request $request, $id)
    {
        ...
        if($course['type']=='product' && !is_null($type) && !is_null($num)){
            $course['price']='99999999999';
            $course['coinPrice']='99999999999';
            $course['travelDetail']=json_decode($course['travelDetail'],true);
            foreach ($course['travelDetail']['carousel'] as $kk){
                if($kk['title']==$type){
                    //验证库存
                    if($num>0 && $num <= intval($kk['number'])){
                        $course['price'] = $course['coinPrice']= $kk['price'] * $num;
                        $course['ProType']=$type;
                        $course['ProNum']=$num;
                    }else{
                        $course['price']='99999999999';
            ...
            if(!is_null($type) && !is_null($num)){
                ...
                foreach ($course['travelDetail']['carousel'] as $kk){
                    if($kk['title']==$type){
                        //验证库存
                        if( $num<=0 || $num > intval($kk['number'])){
                            return $this->createMessageResponse('error', '库存错误，不能购买。');
                        }
                    }
                }
            }
