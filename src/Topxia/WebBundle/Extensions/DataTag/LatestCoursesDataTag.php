...
class LatestCoursesDataTag extends CourseBaseDataTag implements DataTag  
{
    ...
    {	
        ...
        if(isset($arguments['excludeCategoryCode'])){
            $category = $this->getCategoryService()->getCategoryByCode($arguments['excludeCategoryCode']);
            $childIds = $this->getCategoryService()->findCategoryChildrenIds($category['id']);
            $childIds[]=$category['id'];
            $conditions['excludeCategoryIds'] = $childIds;
        }
